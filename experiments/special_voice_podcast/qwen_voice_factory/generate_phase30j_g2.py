#!/usr/bin/env python3
"""Manual Phase 30J Generation 2 Qwen VoiceDesign source generator."""
...
def main() -> int:
    ...
    if set(PROMPTS.get("candidates", {})) != set(IDENTITIES):
        raise SystemExit("six G2 prompts are required")
    before = memory()
    say("[Phase30J] Offline mode enabled. CPU-only Qwen VoiceDesign runner.")
    say(f"[Phase30J] MemAvailable: {before['ram_available_bytes'] / 2**30:.2f} GiB; swap used: {before['swap_used_bytes'] / 2**30:.2f} GiB.")
    ...
    if before["ram_available_bytes"] < 8 * 2**30:
        say("[Phase30J] WARNING: MemAvailable is below preferred 8 GiB; continuing under the documented RAM-risk allowance.")
    OUT.mkdir(parents=True, exist_ok=True)
    MEASUREMENTS.parent.mkdir(parents=True, exist_ok=True)
    say("[Phase30J] Loading Qwen model...")
    started = time.perf_counter()
    model = Qwen3TTSModel.from_pretrained(str(MODEL), device_map="cpu", dtype=torch.bfloat16, attn_implementation="eager")
    after_load = memory()
    say(f"[Phase30J] Model loaded in {time.perf_counter() - started:.2f}s.")
    records = []
    try:
        for number, identity in enumerate(IDENTITIES, start=1):
            target = OUT / f"g2_{identity[-1]}.wav"
            if target.is_file():
                existing = analyze(target)
                if valid(existing):
                    say(f"[{number}/6] {identity} already exists and is technically valid; preserving it.")
                    records.append({"identity": identity, "preserved_existing": True, "prompt": PROMPTS["candidates"][identity], "audio": existing})
                    continue
                say(f"[{number}/6] {identity} has an invalid existing output; regenerating after retaining it until a valid replacement is ready.")
            say(f"[{number}/6] {identity} ...")
            record = render(model, identity, target)
            records.append(record)
            say(f"[{number}/6] {identity} saved. Duration: {record['audio']['duration_seconds']:.2f}s; generation time: {record['generation_seconds']:.2f}s.")
    finally:
        del model
        gc.collect()
    ...
    say("[Phase30J] All six Generation 2 source voices finished.")
    return 0
